<table style="width: 100%; color: #444; border-spacing: 0">
    ...
    <tbody>
        ...
        foreach ($tasks as $task) {
            ...
            $project_id = get_array_value($task, "project_id");
            $project_title = get_array_value($task, "project_title");
            ?>
            <tr>
                <td style="border: 1px solid #ddd; padding: 8px 10px; border-top: none; border-right: none"><?php echo anchor(get_uri("projects/view/$project_id/tasks?task=$task_id"), lang("task") . " " . $task_id); ?></td>
