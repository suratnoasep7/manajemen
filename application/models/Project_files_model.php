...
class Project_files_model extends Crud_model {
    ...
    function get_detail_clients($options = array()) {
        $project_files_table = $this->db->dbprefix('project_files_clients');
        $users_table = $this->db->dbprefix('users');
        $where = "";
        $id = get_array_value($options, "id");
        if ($id) {
    ...
    function get_data_document($document_no) {
        $sql = "SELECT document_no from project_files WHERE document_no = '".$document_no."' ";
        return $this->db->query($sql);
    }
