...
class Dashboard extends MY_Controller {
    ...
    public function index() {
        ...
        if ($this->login_user->user_type === "staff" && $this->show_staff_on_staff) {
            ...
            $view_data["total_project"] = $this->Dashboards_model->get_data_project();
            $view_data["total_task"] = $this->Dashboards_model->get_data_task();
            $view_data["total_member"] = $this->Dashboards_model->get_data_team_member();
            $view_data["total_clients"] = $this->Dashboards_model->get_data_clients();
            $view_data["total_document"] = $this->Dashboards_model->get_data_document();
            $view_data["total_dokumen_pdf"] = $this->Dashboards_model->get_data_document_pdf("pdf")->num_rows();
            $view_data["total_dokumen_doc"] = $this->Dashboards_model->get_data_document_pdf("doc")->num_rows();
            $view_data["total_dokumen_zip"] = $this->Dashboards_model->get_data_document_pdf("zip")->num_rows();
            $view_data["total_dokumen_rar"] = $this->Dashboards_model->get_data_document_pdf("rar")->num_rows();


            $this->template->rander("dashboards/index", $view_data);
        } else if($this->login_user->user_type == "client") {
            $view_data["total_project"] = $this->Dashboards_model->get_data_project();
            $this->template->rander("dashboards/clients", $view_data);
        } else if($this->login_user->user_type == "engineer" || $this->login_user->user_type == "reviewer") {
            $view_data["show_timeline"] = get_array_value($widgets, "new_posts");
            $view_data["show_attendance"] = get_array_value($widgets, "clock_in_out");
            $view_data["show_event"] = get_array_value($widgets, "events_today");
            $view_data["show_project_timesheet"] = get_array_value($widgets, "my_timesheet_statistics");
