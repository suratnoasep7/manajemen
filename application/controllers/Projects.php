...
class Projects extends MY_Controller {
    ...
    function all_projects() {
        ...
        if ($this->login_user->user_type == "staff" || $this->login_user->user_type == "engineer") {
    ...
    function files_client($project_id) {
        $options = array("project_id" => $project_id);
        $view_data['files'] = $this->Project_files_model->get_details($options)->result();
        $view_data['project_id'] = $project_id;
        $this->load->view("projects/files_client/index", $view_data);
    }

    function view_file_client($file_id = 0) {
        $file_info = $this->Project_files_model->get_detail_clients(array("id" => $file_id))->row();
        ...
        if ($file_info) {
            ...
            $file_url = base_url()."assets/file/".$file_info->file_name;
    ...
    function file_modal_form_client() {
        $view_data['model_info'] = $this->Project_files_model->get_one($this->input->post('id'));
        $project_id = $this->input->post('project_id') ? $this->input->post('project_id') : $view_data['model_info']->project_id;

        $view_data['project_id'] = $project_id;
        $this->load->view('projects/files_client/modal_form', $view_data);
    }
    ...
    function save_file() {
        ...
        $dataProject = $this->Project_files_model->get_data_document($this->input->post('document_no'))->result_array();
            ...
            if($ukuran < 1044070){
                if ($terupload) {
                    ...
                    $data = array(
                        ...
                        "disicpline" => $this->input->post('disicpline'),
                    ...
                    $result= $this->Project_files_model->save($data);
                    if($result) {
    ...
    function save_file_client() {
        ...
        if($ukuran < 1044070){
            if ($terupload) {
                ...
                $result= $this->db->insert('project_files_clients',$data);;
                if($result) {
                    redirect("projects/view/". $project_id);
                } else {
                    redirect("projects/view/". $project_id);
    ...
    function files_list_data_client($project_id = 0) {

        $options = array("project_id" => $project_id);
        $list_data = $this->Project_files_model->get_detail_clients($options)->result();
        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_file_row_client($data);
        }
        echo json_encode(array("data" => $result));
    }
    ...
    private function _make_file_row_client($data) {
        $file_icon = get_file_icon(strtolower(pathinfo($data->file_name, PATHINFO_EXTENSION)));
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "1", "data-url" => get_uri("projects/view_file_client/" . $data->id)));
